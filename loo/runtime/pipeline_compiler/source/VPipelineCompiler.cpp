...
#	endif
		...
		*/
		...
		{
			switch (uint32_t(srcFormat &EShaderLangFormat::_ApiStorageFormatMask))
			{
			case uint32_t(EShaderLangFormat::OpenGL | EShaderLangFormat::HighLevel):
			case uint32_t(EShaderLangFormat::Vulkan | EShaderLangFormat::HighLevel):
				return true;
			}
			return false;
		}
		...
		*/
		...
		{
			switch (uint32_t(dstFormat & EShaderLangFormat::_ApiStorageFormatMask))
			{
			case uint32_t(EShaderLangFormat::Vulkan | EShaderLangFormat::SPIRV):			return true;
			case uint32_t(EShaderLangFormat::Vulkan | EShaderLangFormat::ShaderModule):	return hasDevice;
			}
			return false;
		}
		...
		*/
		...
		=================================================
		*/
		struct MergeUniformDataVisitLocal
		{
			const PipelineDescription::Uniform &src;
			PipelineDescription::Uniform &dst;
			MergeUniformDataVisitLocal (const PipelineDescription::Uniform &src_,
				PipelineDescription::Uniform &dst_) :src(src_),dst (dst_) {}

			bool operator()(const PipelineDescription::Texture &lhs)const
			{
			...
			bool operator()(const PipelineDescription::Sampler &)const
			{
			...
			bool operator()(const PipelineDescription::SubpassInput &lhs)const
			{
			...
			bool operator()(const PipelineDescription::Image &lhs)const
			{
			...
			bool operator() (const PipelineDescription::UniformBuffer &lhs)const
			{
			...
			bool operator()(const PipelineDescription::StorageBuffer &lhs)const
			{
			...
			bool operator()(const PipelineDescription::RayTracingScene &lhs)const
			{
		...
		ND_ static bool  MergeUniformData (const PipelineDescription::Uniform &src, INOUT PipelineDescription::Uniform &dst)
		{
			...
			auto visitfunc = MergeUniformDataVisitLocal (src, dst);
...
# endif
		...
		*/
		...
		{
			FixedArray< PipelineDescription::Uniform *, LOO_MaxBufferDynamicOffsets >	sorted;

			for (auto& ds_layout : descriptorSets)
			{
				for (auto& un : *ds_layout.uniforms)
				{
					auto* ubuf = UnionGetIf< PipelineDescription::UniformBuffer > (&un.second.data);
					auto* sbuf = UnionGetIf< PipelineDescription::StorageBuffer > (&un.second.data);
					if (ubuf and EnumEq (ubuf->state, EResourceState::_BufferDynamicOffset))
					{
						sorted.push_back (const_cast<PipelineDescription::Uniform *>(&un.second));
					}
					else if (sbuf and EnumEq (sbuf->state, EResourceState::_BufferDynamicOffset))
					{
						sorted.push_back (const_cast<PipelineDescription::Uniform *>(&un.second));
					}
				}
			...
			CHECK (index <= LOO_MaxBufferDynamicOffsets);
		}
			...
			for (auto sh_iter = shader.data.begin (); sh_iter != shader.data.end ();)
			{
				...
				case EShaderLangFormat::SPIRV: {
					...
					if (iter == _shaderCache.end ())
					{
						...
						auto	module = loo::global::MakeSharedPtr<VCachedDebuggableShaderModule> (shader_id, spv_data);
