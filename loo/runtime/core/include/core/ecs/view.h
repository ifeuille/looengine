...
namespace looecs
{
	...
	class PersistentView final 
	{
		...
		PersistentView (view_type &view, pool_type<Component> &... pools) ECS_NOEXCEPT
	...
	public:
		...
		const entity_type & operator[](const size_type pos) const ECS_NOEXCEPT {
			return view[pos];
		}

		bool contains (const entity_type entity) const ECS_NOEXCEPT {
			return view.has (entity) && (view.data ()[view.get (entity)] == entity);
		}

		template<typename Comp>
		const Comp & get (const entity_type entity) const ECS_NOEXCEPT {
			assert (contains (entity));
			return std::get<pool_type<Comp> &> (pools).get (entity);
		}

		template<typename Comp>
		inline Comp & get (const entity_type entity) ECS_NOEXCEPT {
	...
	class View final {
		...
		class Iterator {
			...
			Iterator (unchecked_type unchecked, underlying_iterator_type begin, underlying_iterator_type end) ECS_NOEXCEPT
				: unchecked{ unchecked },
				begin{ begin },
				end{ end },
			...
			extent_type min (std::index_sequence<Indexes...>) const ECS_NOEXCEPT {
				return std::min ({ std::get<Indexes> (unchecked)->extent ()... });
			}

			bool valid () const ECS_NOEXCEPT {
		...
		View (pool_type<Component> &... pools) ECS_NOEXCEPT
			: pools{ pools... }
		{}

		template<typename Comp>
		const pool_type<Comp> & pool () const ECS_NOEXCEPT {
			return std::get<pool_type<Comp> &> (pools);
		}

		const view_type * candidate () const ECS_NOEXCEPT {
			return std::min ({ static_cast<const view_type *>(&pool<Component> ())... }, [](const auto *lhs, const auto *rhs) {
				return lhs->size () < rhs->size ();
			});
		}

		unchecked_type unchecked (const view_type *view) const ECS_NOEXCEPT {
		...
		inline std::enable_if_t<std::is_same<Comp, Other>::value, const Other &>
			get (const component_iterator_type<Comp> &it, const Entity) const ECS_NOEXCEPT { return *it; }

		template<typename Comp, typename Other>
		inline std::enable_if_t<!std::is_same<Comp, Other>::value, const Other &>
			get (const component_iterator_type<Comp> &, const Entity entity) const ECS_NOEXCEPT { return pool<Other> ().get (entity); }
		...
		public:
			...
			inline iterator_type end () ECS_NOEXCEPT {
				return cend ();
			}
	...
	class View<Entity, Component> final {
		...
		View (pool_type &pool) ECS_NOEXCEPT
			: pool{ pool }
		{}
	...
	public:
		...
		const raw_type * raw () const ECS_NOEXCEPT {
			return pool.raw ();
		}

		inline raw_type * raw () ECS_NOEXCEPT {
	...
	class RawView final {
		...
		RawView (pool_type &pool) ECS_NOEXCEPT
			: pool{ pool }
		{}
	...
	public:
		...
		const raw_type & operator[](const size_type pos) const ECS_NOEXCEPT {
			return pool[pos];
		}

		inline raw_type & operator[](const size_type pos) ECS_NOEXCEPT {
	...
	class RuntimeView {
		...
		class Iterator {
			...
			Iterator (underlying_iterator_type begin, underlying_iterator_type end, const view_type * const *first, const view_type * const *last, extent_type extent) ECS_NOEXCEPT
				: begin{ begin },
				end{ end },
				first{ first },
				last{ last },
		...
		RuntimeView (pattern_type others) ECS_NOEXCEPT
		...
		extent_type min () const ECS_NOEXCEPT {
			extent_type extent{};
		...
		inline bool valid () const ECS_NOEXCEPT {
