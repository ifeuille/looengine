...
namespace looecs
{
	namespace internal
	{
		...
		struct NullCollector final {
			...
			bool operator()(result_type) const ECS_NOEXCEPT { return true; }
		};
		...
		struct NullCollector<void> final {
			...
			bool operator()() const ECS_NOEXCEPT { return true; }
		};
	...
	class Sink<Ret (Args...)> final {
		...
		Sink (std::vector<call_type> *calls) ECS_NOEXCEPT
			: calls{ calls }
		{}
	...
	public:
		 ...
		 */
		bool operator==(const SigH &other) const ECS_NOEXCEPT {
	 ...
	 */
	template<typename Ret, typename... Args>
	bool operator!=(const SigH<Ret (Args...)> &lhs, const SigH<Ret (Args...)> &rhs) ECS_NOEXCEPT {
		return !(lhs == rhs);
	}
