...
namespace loo
{
	namespace vkfg
	{
		...
		=================================================
		*/
		struct ConvertClearValue_VisitFunc
		{
			VkClearValue & result;
			ConvertClearValue_VisitFunc (VkClearValue & r) :result (r) {}
			void operator() (const loo::math::RGBA32f &src)const { MemCopy (result.color.float32, src); }
			void operator()(const loo::math::RGBA32u &src)const { MemCopy (result.color.uint32, src); }
			void operator()(const loo::math::RGBA32i &src)const { MemCopy (result.color.int32, src); }
			void operator()(const loo::math::DepthStencil &src)const { result.depthStencil = { src.depth, src.stencil }; }
			void operator()(const NullUnion &)const { memset (&result, 0, sizeof (result)); }
		};
		inline void ConvertClearValue (const RenderPassDesc::ClearValue_t &cv, OUT VkClearValue &result)
		{
			ConvertClearValue_VisitFunc vf (result);
			Visit (cv,
#if 1
				vf
#else
				[&result](const loo::math::RGBA32f &src) { MemCopy (result.color.float32, src); },
				[&result](const loo::math::RGBA32u &src) { MemCopy (result.color.uint32, src); },
				[&result](const loo::math::RGBA32i &src) { MemCopy (result.color.int32, src); },
				[&result](const loo::math::DepthStencil &src) { result.depthStencil = { src.depth, src.stencil }; },
		...
		*/
		bool VLogicalRenderPass::Create (VCommandBuffer &fgThread, const RenderPassDesc &desc)
		{
			...
			if (samples.has_value ())
			{
				...
				{
					//LOO_LOGD( "Render pass attachment sample count was changed, sample mask updated" );
			...
			for (auto& src : desc.viewports)
			{
				...
				// scissor
				VkRect2D		rect;
				rect.offset.x = loo::math::RoundToInt (src.rect.left);
				rect.offset.y = loo::math::RoundToInt (src.rect.top);
				rect.extent.width = loo::math::RoundToInt (src.rect.Width ());
				rect.extent.height = loo::math::RoundToInt (src.rect.Height ());
				...
				if (enable_sri)
				{
					...
					palette.shadingRatePaletteEntryCount = loo::math::Max (1u, uint (src.palette.size ()));
