...
#include "vkfg/vulkan/commandbuffer/vtaskgraph.hpp"
	...
	namespace vkfg
	{
		...
		*/
		bool VCommandBuffer::Begin (const CommandBufferDesc &desc, const VCmdBatchPtr &batch, VDeviceQueueInfoPtr queue)
		{
			...
			// create command pool
			{
				...
				_perQueue.resize (loo::math::Max (_perQueue.size (), index + 1));
		...
		*/
		Task  VCommandBuffer::AddTask (const UpdateImage &task)
		{
			...
			if (All (task.imageSize == loo::math::uint3 (0)))
				return null;	// TODO: is it an error?
		...
		*/
		Task  VCommandBuffer::_AddUpdateImageTask (const UpdateImage &task)
		{
			...
			ASSERT (Any (task.imageSize > loo::math::uint3 (0)));

			const loo::math::uint3		image_size = Max (task.imageSize, 1u);
			...
			const BytesU	row_pitch = loo::math::Max (task.dataRowPitch, BytesU (image_size.x * block_size + block_dim.x - 1) / (block_dim.x * 8));
			const BytesU	min_slice_pitch = (image_size.y * row_pitch + block_dim.y - 1) / block_dim.y;
			const BytesU	slice_pitch = loo::math::Max (task.dataSlicePitch, min_slice_pitch);
			...
			if (total_size < min_size)
			{
				...
				for (BytesU readn; readn < total_size;)
				{
					...
					copy.AddRegion (off, row_length, img_height,
						...
						task.imageOffset + loo::math::int3 (0, 0, z_offset), loo::math::uint3 (image_size.x, image_size.y, z_size));
				...
				for (uint slice = 0; slice < image_size.z; ++slice)
				{
					...
					for (BytesU readn; readn < ArraySizeOf (data);)
					{
						...
						copy.AddRegion (off, row_length, y_size,
							ImageSubresourceRange{ task.mipmapLevel, task.arrayLayer, 1, task.aspectMask },
							task.imageOffset + loo::math::int3 (0, y_offset, slice), loo::math::uint3 (image_size.x, y_size, 1));
		...
		*/
		Task  VCommandBuffer::AddTask (const ReadImage &task)
		{
			...
			if (loo::math::All (task.imageSize == loo::math::uint3 (0)))
				return null;	// TODO: is it an error?
		...
		*/
		Task  VCommandBuffer::AddTask (const BuildRayTracingScene &task)
		{
			...
			result->_hitShadersPerInstance = loo::math::Max (1u, task.hitShadersPerInstance);
		...
		*/
		LogicalPassID  VCommandBuffer::CreateRenderPass (const RenderPassDesc &desc)
		{
			...
			_rm.logicalRenderPassCount = loo::math::Max (uint (index) + 1, _rm.logicalRenderPassCount);
		...
		*/
		...
		inline Res const*  VCommandBuffer::_ToLocal (ID id, INOUT LocalResPool<Res, MainPool, MC> &localRes, StringView msg)
		{
			...
			localRes.maxLocalIndex = loo::math::Max (uint (local) + 1, localRes.maxLocalIndex);
			localRes.maxGlobalIndex = loo::math::Max (uint (id.Index ()) + 1, localRes.maxGlobalIndex);
		...
		*/
		bool VCommandBuffer::_StorePartialData (ArrayView<uint8_t> srcData, const BytesU srcOffset, OUT RawBufferID &dstBuffer, OUT BytesU &dstOffset, OUT BytesU &size)
		{
			...
			const BytesU	min_size = loo::math::Min ((src_size + MaxBufferParts - 1) / MaxBufferParts, loo::math::Min (src_size, MinBufferPart));
			void *			ptr = null;
