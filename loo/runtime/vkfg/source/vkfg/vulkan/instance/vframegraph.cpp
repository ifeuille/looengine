	...
	namespace vkfg
	{
		...
		*/
		void  VFrameGraph::Deinitialize ()
		{
			...
			{
				LOO_LOGD ("Max command buffers "s << ToString (_cmdBufferPool.CreatedObjectsCount ()));
				LOO_LOGD ("Max command batches "s << ToString (_cmdBatchPool.CreatedObjectsCount ()));
				LOO_LOGD ("Max submitted batches "s << ToString (_submittedPool.CreatedObjectsCount ()));
		...
		*/
		...
		=================================================
		*/
		struct CreateSwapchain_VisitHelper
		{
			VResourceManager& _resourceMngr;
			RawSwapchainID& oldSwapchain;
			VFrameGraph& _this;
			StringView& dbgName;
			CreateSwapchain_VisitHelper (VResourceManager& res, RawSwapchainID& olds, VFrameGraph& t, StringView& dn)
				:_resourceMngr (res), oldSwapchain (olds), _this (t), dbgName (dn) {}

			RawSwapchainID operator()(const VulkanSwapchainCreateInfo &info)const
			{
				return _resourceMngr.CreateSwapchain (info, oldSwapchain, _this, dbgName);
			}
			template<typename T>
			RawSwapchainID operator()(const T &)const {
				ASSERT (!"not supported");
				return RawSwapchainID ();
			}
		};
		SwapchainID  VFrameGraph::CreateSwapchain (const SwapchainCreateInfo_t &desc, RawSwapchainID oldSwapchain, StringView dbgName)
		{
			CHECK_ERR (_IsInitialized ());
			CreateSwapchain_VisitHelper visitfunc (_resourceMngr, oldSwapchain, *this, dbgName);
		...
		*/
		bool  VFrameGraph::MapBufferRange (RawBufferID id, BytesU offset, INOUT BytesU &size, OUT void* &dataPtr)
		{
			...
			size = loo::math::Min (size, mem_info.size - offset);
		...
		*/
		...
		{
			for (size_t a = 0, a_max = loo::math::Min (maxIter, _queueMap.size ()), changed = 1;
				changed and (a < a_max);
				++a)
			{
				changed = 0;
		...
		*/
		bool  VFrameGraph::_FlushQueue (EQueueType queueIndex, uint maxIter)
		{
			...
			for (size_t b = 0, b_max = loo::math::Min (maxIter, q.pending.size ()), changed = 1;
				changed and (b < b_max);
				++b)
			{
				changed = 0;
			...
			for (auto iter = q.submitted.begin (); iter != q.submitted.end ();)
			{
				...
				{
					{
						EXLOCK (std::mutex,_statisticGuard);
