	...
	namespace vkfg
	{
		...
		*/
			...
			_allReadableStages{ VK_PIPELINE_STAGE_ALL_COMMANDS_BIT }
		{
			...
			// validate constants
			CHECK (GetDeviceLimits ().maxPushConstantsSize >= LOO_MaxPushConstantsSize);
			CHECK (GetDeviceLimits ().maxVertexInputAttributes >= LOO_MaxVertexAttribs);
			CHECK (GetDeviceLimits ().maxVertexInputBindings >= LOO_MaxVertexBuffers);
			CHECK (GetDeviceLimits ().maxViewports >= LOO_MaxViewports);
			CHECK (GetDeviceLimits ().maxColorAttachments >= LOO_MaxColorBuffers);
			CHECK (GetDeviceLimits ().maxBoundDescriptorSets >= LOO_MaxDescriptorSets);
			CHECK ((GetDeviceLimits ().maxDescriptorSetUniformBuffersDynamic + GetDeviceLimits ().maxDescriptorSetStorageBuffersDynamic) >= LOO_MaxBufferDynamicOffsets);
		}
