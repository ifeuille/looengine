	...
	namespace vkfg
	{
		...
		*/
			...
			EQueueFamilyMask queueFamilyMask, EResourceState defaultState, StringView dbgName)
		{
			...
			const bool	opt_tiling = not uint (uint(memObj.MemoryType ()) &uint( EMemoryTypeExt::HostVisible));
		...
		*/
		VkImageView  VImage::GetView (const VDevice &dev, const HashedImageViewDesc &desc) const
		{
			...
			{
				SHAREDLOCK (std::shared_mutex,_viewMapLock);
			...
			EXLOCK (std::shared_mutex, _viewMapLock);
			std::pair< ImageViewMap_t::iterator,bool> ret = _viewMap.insert ({ desc, VK_NULL_HANDLE });
			if (not ret.second)
				return ret.first->second;
			//auto[iter, inserted] = _viewMap.insert ({ desc, VK_NULL_HANDLE });

			CHECK_ERR (_CreateView (dev, desc, OUT ret.first->second));
		...
		*/
		bool VImage::_CreateView (const VDevice &dev, const HashedImageViewDesc &viewDesc, OUT VkImageView &outView) const
		{
			...
			const loo::math::uint4				swizzle = loo::math::Min (loo::math::uint4 (uint (CountOf (components) - 1)), desc.swizzle.ToVec ());
