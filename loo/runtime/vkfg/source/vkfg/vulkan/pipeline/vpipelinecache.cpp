	...
	namespace vkfg
	{
		...
		*/
			...
			_pipelinesCache{ VK_NULL_HANDLE }
		{
			...
			_tempSpecialization.reserve (max_stages * LOO_MaxSpecConstants);
			_tempSpecEntries.reserve (max_stages * LOO_MaxSpecConstants);
		...
		*/
		struct _SetupShaderDebugging_Helper
		{
			...
			_SetupShaderDebugging_Helper (const VkShaderStageFlags	s,
			EShaderDebugMode &d) :stages(s), debugMode(d){}

			template<typename ShaderMod>
			inline bool isContinue (const ShaderMod& sh)
			{
				return (not EnumEq (stages, sh.stage) or sh.debugMode != debugMode);
			}

			template<>
			inline bool isContinue <VComputePipeline::ShaderModule> (const VComputePipeline::ShaderModule& sh)
			{
				return (sh.debugMode != debugMode);
			}
			...
			OUT EShaderDebugMode &debugMode, OUT EShaderStages &debuggableShaders, OUT RawPipelineLayoutID &layoutId)
		{
			...
			_SetupShaderDebugging_Helper helper (stages, debugMode);
			for (auto& sh : ppln._shaders)
			{
				if (helper.isContinue (sh))
				{
					continue;
				}
		...
		*/
			...
			OUT VPipelineLayout const*	&outLayout)
		{
			...
			{
				SHAREDLOCK (std::shared_mutex,gppln._instanceGuard);
			...
			_SetViewportState (OUT viewport_info, OUT _tempViewports, OUT _tempScissors,
				loo::math::uint2 (1024, 1024), inst.viewportCount, inst.dynamicState);
			...
			{
				EXLOCK (std::shared_mutex,gppln._instanceGuard);
				std::pair< VGraphicsPipeline::Instances_t::iterator,bool> ret = gppln._instances.insert ({ std::move (inst), outPipeline });
		...
		*/
			...
			OUT VPipelineLayout const*	&outLayout)
		{
			...
			// find existing instance
			{
				SHAREDLOCK (std::shared_mutex, mppln._instanceGuard);
			...
			// try to insert new instance
			{
				EXLOCK (std::shared_mutex, mppln._instanceGuard);

				std::pair< VMeshPipeline::Instances_t::iterator, bool> ret = mppln._instances.insert ({ std::move (inst), outPipeline });
		...
		*/
			...
			OUT VPipelineLayout const*	&outLayout)
		{
			...
			ASSERT (not localGroupSize.has_value () or Any (cppln._localSizeSpec != loo::math::uint3 (ComputePipelineDesc::UNDEFINED_SPECIALIZATION)));
			...
			// find existing instance
			{
				SHAREDLOCK (std::shared_mutex, cppln._instanceGuard);
			...
			// try to insert new instance
			{
				EXLOCK (std::shared_mutex, cppln._instanceGuard);

				std::pair< VComputePipeline::Instances_t::iterator, bool> ret = cppln._instances.insert ({ std::move (inst), outPipeline });
		...
		*/
			...
			OUT BufferCopyRegions_t		&copyRegions)
		{
			...
			SHAREDLOCK (std::shared_mutex, scene_data.guard);
			...
			for (auto& shader : shaderGroups)
			{
				...
				case EGroupType::MissShader:
					miss_shader_count = loo::math::Max (shader.offset + 1, miss_shader_count);
					break;
			...
			const BytesU	req_size = loo::math::AlignToLarger (shaderTable._blockSize, alignment) * debug_modes.size ();
			...
			for (auto mode : debug_modes)
			{
				// create ray-gen shader
				{
					...
					_tempStages.emplace_back (stage);
				...
				DEBUG_ONLY (
					...
					offset = loo::math::AlignToLarger (offset + shaderTable._blockSize, alignment);
		...
		*/
			...
			OUT VkPipelineShaderStageCreateInfo &stage)
		{
				...
				shader.stage == VK_SHADER_STAGE_MISS_BIT_NV)
			{
				for (auto& spec : _rtShaderSpecs)
				{
					...
					if (iter != shader.specConstants.end ())
					{
						...
						_tempSpecEntries.emplace_back (entry);
					}
				}
			}
			...
			if (_tempSpecEntries.size () > entry_count)
			{
				...
				_tempSpecialization.emplace_back (spec_info);
			}
		...
		*/
			...
			const uint subpassIndex) const
		{
			...
			for (size_t i = 0, cnt = loo::math::Min (inState.buffers.size (), attachments.size ()); i < cnt; ++i)
			{
		...
		*/
		void VPipelineCache::_ValidateRenderState (const VDevice &dev, INOUT RenderState &renderState, INOUT EPipelineDynamicState &dynamicStates) const
		{
				...
				t = EPipelineDynamicState (uint (t) << 1))
			{
				...
				case EPipelineDynamicState::_end:
		...
		*/
		...
		{
			const loo::math::bool3	has_spec = (localSizeSpec != loo::math::uint3 (ComputePipelineDesc::UNDEFINED_SPECIALIZATION));
			//ASSERT(Any( has_spec ));
