...
namespace loo
{
	namespace vkfg
	{
		...
		*/
		...
		{
			LOO_LOGI ("apiVersion:    "s << ToString (VK_VERSION_MAJOR (GetDeviceProperties ().apiVersion)) << '.' <<
				ToString (VK_VERSION_MINOR (GetDeviceProperties ().apiVersion)) << '.' <<
				ToString (VK_VERSION_PATCH (GetDeviceProperties ().apiVersion)));

			LOO_LOGI ("driverVersion: "s << ToString (VK_VERSION_MAJOR (GetDeviceProperties ().driverVersion)) << '.' <<
				ToString (VK_VERSION_MINOR (GetDeviceProperties ().driverVersion)));

			LOO_LOGI ("vendorName:    "s << _GetVendorNameByID (GetDeviceProperties ().vendorID).data());
			LOO_LOGI ("deviceName:    "s << GetDeviceProperties ().deviceName);
		}
