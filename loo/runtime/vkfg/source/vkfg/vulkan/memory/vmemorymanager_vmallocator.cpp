	...
	namespace vkfg
	{
		...
		*/
		VkMemoryPropertyFlags  VMemoryManager::VulkanMemoryAllocator::_ConvertToMemoryProperties (EMemoryType memType)
		{
			...
			for (EMemoryTypeExt t = EMemoryTypeExt (1 << 0); t < EMemoryTypeExt::_end; t = EMemoryTypeExt (uint (t) << 1))
			{
		...
		*/
		bool VMemoryManager::VulkanMemoryAllocator::_CreateAllocator (OUT VmaAllocator &alloc) const
		{
			...
			info.preferredLargeHeapBlockSize = VkDeviceSize (LOO_VkDevicePageSizeMb) << 20;
...
#endif	// LOO_ENABLE_VULKAN_MEMORY_ALLOCATOR
