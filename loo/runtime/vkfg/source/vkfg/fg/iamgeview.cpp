...
*/
		template <uint Bits>
		LOO_FORCEINLINE float ScaleUNorm (uint value)
		{
		...
		*/
		template <uint Bits>
		LOO_FORCEINLINE float ScaleSNorm (int value)
		{
			STATIC_ASSERT (Bits <= 32, "");
		...
		*/
#if 1
		struct ReadUIntScalar
		{
			const uint Bits;
			const uint OffsetBits;
			ReadUIntScalar (const uint& bits, const uint& offsetBits) :Bits (bits), OffsetBits (offsetBits) {}
			LOO_FORCEINLINE int operator()(const StaticArray<uint, 4> &data)
			{
				ASSERT (Bits <= 32);
				ASSERT (Bits + (OffsetBits & 31) <= 32);
				...
				{
					const uint	mask = (~0u >> (32 - Bits));
					const uint	offset = (OffsetBits % 32);
					const uint	index = (OffsetBits / 32);
		...
		LOO_FORCEINLINE uint ReadUIntScalar (const StaticArray<uint, 4> &data)
		{
			STATIC_ASSERT (Bits <= 32);
		...
		*/
		...
		static void ReadInt (ArrayView<ImageView::T> pixel, OUT loo::math::RGBA32i &result)
		{
			StaticArray< uint, 4 >	bits;
			std::memcpy (bits.data (), pixel.data (), std::min ((R + G + B + A + 7) / 8, uint (ArraySizeOf (pixel))));
		...
		*/
		template <uint R, uint G, uint B, uint A>
		static void ReadUInt (ArrayView<ImageView::T> pixel, OUT loo::math::RGBA32u &result)
		{
			StaticArray< uint, 4 >	bits;
			std::memcpy (bits.data (), pixel.data (), std::min ((R + G + B + A + 7) / 8, uint (ArraySizeOf (pixel))));
		...
		*/
		template <uint R, uint G, uint B, uint A>
		static void ReadUNorm (ArrayView<ImageView::T> pixel, OUT loo::math::RGBA32f &result)
		{
			loo::math::RGBA32u		c;
		...
		*/
		template <uint R, uint G, uint B, uint A>
		static void ReadSNorm (ArrayView<ImageView::T> pixel, OUT loo::math::RGBA32f &result)
		{
			loo::math::RGBA32i		c;
		...
		*/
...
#if 1
		template <>
		LOO_FORCEINLINE void ReadFloat<16,0,0,0> (ArrayView<ImageView::T> pixel, OUT loo::math::RGBA32f &result)
		{
			StaticArray< HalfBits, 4 >	src = {};
			std::memcpy (src.data (), pixel.data (), std::min ((16 + 0 + 0 + 0 + 7) / 8, int (ArraySizeOf (pixel))));
		...
		template <>
		LOO_FORCEINLINE void ReadFloat<16, 16, 0, 0> (ArrayView<ImageView::T> pixel, OUT loo::math::RGBA32f &result)
		{
			StaticArray< HalfBits, 4 >	src = {};
			std::memcpy (src.data (), pixel.data (), std::min ((16 + 16 + 0 + 0 + 7) / 8, int (ArraySizeOf (pixel))));
		...
		template <>
		LOO_FORCEINLINE void ReadFloat<16, 16, 16, 0> (ArrayView<ImageView::T> pixel, OUT loo::math::RGBA32f &result)
		{
			StaticArray< HalfBits, 4 >	src = {};
			std::memcpy (src.data (), pixel.data (), std::min ((16 + 16 + 16 + 0 + 7) / 8, int (ArraySizeOf (pixel))));
		...
		template <>
		LOO_FORCEINLINE void ReadFloat<16, 16, 16, 16> (ArrayView<ImageView::T> pixel, OUT loo::math::RGBA32f &result)
		{
			StaticArray< HalfBits, 4 >	src = {};
			std::memcpy (src.data (), pixel.data (), std::min ((16 + 16 + 16 + 16 + 7) / 8, int (ArraySizeOf (pixel))));
		...
		template <>
		LOO_FORCEINLINE void ReadFloat<32, 0, 0, 0> (ArrayView<ImageView::T> pixel, OUT loo::math::RGBA32f &result)
		{
			result = {};
			std::memcpy (result.data (), pixel.data (), std::min ((32 + 0 + 0 + 0 + 7) / 8, int (ArraySizeOf (pixel))));
		}
		template <>
		LOO_FORCEINLINE void ReadFloat<32, 32, 0, 0> (ArrayView<ImageView::T> pixel, OUT loo::math::RGBA32f &result)
		{
			result = {};
			std::memcpy (result.data (), pixel.data (), std::min ((32 + 32 + 0 + 0 + 7) / 8, int (ArraySizeOf (pixel))));
		}
		template <>
		LOO_FORCEINLINE void ReadFloat<32, 32, 32, 0> (ArrayView<ImageView::T> pixel, OUT loo::math::RGBA32f &result)
		{
			result = {};
			std::memcpy (result.data (), pixel.data (), std::min ((32 + 32 + 32 + 0 + 7) / 8, int (ArraySizeOf (pixel))));
		}
		template <>
		LOO_FORCEINLINE void ReadFloat<32, 32, 32, 32> (ArrayView<ImageView::T> pixel, OUT loo::math::RGBA32f &result)
		{
			result = {};
			std::memcpy (result.data (), pixel.data (), std::min ((32 + 32 + 32 + 32 + 7) / 8, int (ArraySizeOf (pixel))));
		}
#endif
		/*
		=================================================
		...
		=================================================
		*/
		static void ReadFloat_11_11_10 (ArrayView<ImageView::T> pixel, OUT loo::math::RGBA32f &result)
		{
			struct RGBBits
			{
				// Blue //
		...
		=================================================
		*/
		ImageView::ImageView (ArrayView<ArrayView<T>> parts, const loo::math::uint3 &dim, BytesU rowPitch, BytesU slicePitch, EPixelFormat format, EImageAspect aspect) :
