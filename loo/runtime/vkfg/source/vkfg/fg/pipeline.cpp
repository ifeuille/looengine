...
namespace loo
{
	namespace vkfg
	{
		...
		*/
		...
		{
			data.insert (std::make_pair(fmt, loo::global::MakeSharedPtr<ShaderDataImpl<String>> (std::move (src), entry, dbgName)));
		}
		...
		{
			data.insert (std::make_pair( fmt, loo::global::MakeSharedPtr<ShaderDataImpl<Array<uint8_t>>> (std::move (bin), entry, dbgName) ));
		}

		void PipelineDescription::Shader::AddShaderData (EShaderLangFormat fmt, StringView entry, Array<uint> &&bin, StringView dbgName)
		{
			data.insert (std::make_pair( fmt, loo::global::MakeSharedPtr<ShaderDataImpl<Array<uint>>> (std::move (bin), entry, dbgName) ));
		}
		//-----------------------------------------------------------------------------
...
*/
		...
		{
			ASSERT (id.IsDefined ());
			ASSERT (index < LOO_MaxDescriptorSets);
			...
			DEBUG_ONLY (
				...
				ds.uniforms = loo::global::MakeSharedPtr<UniformMap_t> (std::move (uniforms));
