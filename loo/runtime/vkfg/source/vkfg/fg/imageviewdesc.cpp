...
namespace loo
{
	namespace vkfg
	{
		...
		*/
		...
		=================================================
		*/
		static bool ValidateDimension (const EImage imageType, INOUT loo::math::uint3 &dim, INOUT ImageLayer &layers)
		{
			BEGIN_ENUM_CHECKS ();
			switch (imageType)
			{
			...
			case EImage::Tex1D:
			{
				...
				dim = loo::math::Max (loo::math::uint3 (dim.x, 0, 0), 1u);
				layers = 1_layer;
				return true;
			}
			case EImage::Tex2DMS:
			case EImage::Tex2D:
			{
				...
				dim = loo::math::Max (loo::math::uint3 (dim.x, dim.y, 0), 1u);
				layers = 1_layer;
				return true;
			}
			case EImage::TexCube:
			{
				...
				ASSERT (layers.Get () == 6);
				dim = loo::math::Max (loo::math::uint3 (dim.x, dim.y, 1), 1u);
				layers = 6_layer;
				return true;
			}
			...
			case EImage::Tex2DArray:
			{
				...
				ASSERT (dim.z <= 1);
				dim = loo::math::Max (loo::math::uint3 (dim.x, dim.y, 0), 1u);
				layers = loo::math::Max (layers, 1_layer);
				return true;
			}
			case EImage::Tex3D:
			{
				...
				ASSERT (layers.Get () <= 1);
				dim = loo::math::Max (loo::math::uint3 (dim.x, dim.y, dim.z), 1u);
				layers = 1_layer;
				return true;
			}
			case EImage::TexCubeArray:
			{
				...
				dim = loo::math::Max (loo::math::uint3 (dim.x, dim.y, 0), 1u);
				layers = loo::math::Max (layers, 6_layer);
				return true;
			}
			case EImage::Unknown:
		...
		=================================================
		*/
		ND_ static uint  NumberOfMipmaps (const EImage imageType, const loo::math::uint3 &dim)
		{
			BEGIN_ENUM_CHECKS ();
			switch (imageType)
			{
			...
			case EImage::Tex2D:
			case EImage::Tex3D:			return std::ilogb (loo::math::Max (loo::math::Max (dim.x, dim.y), dim.z)) + 1;
			...
			case EImage::TexCube:
			case EImage::TexCubeArray:
			case EImage::Tex2DArray:		return std::ilogb (loo::math::Max (dim.x, dim.y)) + 1;
		...
		*/
		void ImageDesc::Validate ()
		{
			...
			else
			{
				ASSERT (samples <= 1_samples);
				samples = 1_samples;
				maxLevel = MipmapLevel (loo::math::Clamp (maxLevel.Get (), 1u, NumberOfMipmaps (imageType, dimension)));
			}
		}
		//-----------------------------------------------------------------------------
		...
		*/
		void ImageViewDesc::Validate (const ImageDesc &desc)
		{
			...
			baseLayer = ImageLayer{ loo::math::Clamp (baseLayer.Get (), 0u, max_layers - 1) };
			layerCount = loo::math::Clamp (layerCount, 1u, max_layers - baseLayer.Get ());

			baseLevel = MipmapLevel{ loo::math::Clamp (baseLevel.Get (), 0u, desc.maxLevel.Get () - 1) };
			levelCount = loo::math::Clamp (levelCount, 1u, desc.maxLevel.Get () - baseLevel.Get ());
