...
namespace loo
{
	namespace vkfg
	{
		...
		private:
			...
			static constexpr uint	MaxLocalResources = loo::math::Max (VResourceManager::MaxImages, VResourceManager::MaxBuffers);
		...
		public:
			...
			// resource manager //
			template <uint UID>
			ND_ auto const*				AcquireTemporary (loo::_hidden_::ResourceID<UID> id);

			template <uint UID>
			void						ReleaseResource (loo::_hidden_::ResourceID<UID> id);
			...
			ND_ StringView				GetName ()					const { EXLOCK (const DataRaceCheck,_drCheck);  return _dbgName; }
			ND_ VCmdBatch &				GetBatch ()					const { EXLOCK (const DataRaceCheck, _drCheck);  return *_batch; }
			ND_ VCmdBatchPtr const&		GetBatchPtr ()				const { EXLOCK (const DataRaceCheck, _drCheck);  return _batch; }
			ND_ Allocator_t &			GetAllocator () { EXLOCK (DataRaceCheck, _drCheck);  return _mainAllocator; }
			ND_ Statistic_t &			EditStatistic () { EXLOCK (DataRaceCheck, _drCheck);  return _batch->_statistic; }
			ND_ VPipelineCache &		GetPipelineCache () { EXLOCK (DataRaceCheck, _drCheck);  return _pipelineCache; }
			ND_ VBarrierManager &		GetBarrierManager () { EXLOCK (DataRaceCheck, _drCheck);  return _barrierMngr; }
			ND_ Ptr<VLocalDebugger>		GetDebugger () { EXLOCK (DataRaceCheck, _drCheck);  return _debugger.get (); }
			...
			ND_ EQueueFamily			GetQueueFamily ()			const { EXLOCK (const DataRaceCheck, _drCheck);  return _queueIndex; }
		...
		*/
		template <uint UID>
		inline auto const*  VCommandBuffer::AcquireTemporary (loo::_hidden_::ResourceID<UID> id)
		{
			std::pair< VCmdBatch::ResourceMap_t::iterator,bool> ret = _rm.resourceMap.insert (std::make_pair(Resource_t(id), 1));
			//auto[iter, inserted] = _rm.resourceMap.insert ({ Resource_t{ id }, 1 });

			return _instance.GetResourceManager ().GetResource (id, ret.second);
		}
		...
		*/
		template <uint UID>
		inline void  VCommandBuffer::ReleaseResource (loo::_hidden_::ResourceID<UID> id)
		{
			_rm.resourceMap.insert (std::make_pair(Resource_t{ id }, 0)).first->second++;
		}
