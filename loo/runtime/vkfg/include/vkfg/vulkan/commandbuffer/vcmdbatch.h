	...
	namespace vkfg
	{
		...
		public:
			...
			struct Resource
			{
				...
				static constexpr uint		InstanceOffset = sizeof (loo::_hidden_::ResourceID<0>::Index_t) * 8;
				static constexpr uint		IDOffset = InstanceOffset + sizeof (loo::_hidden_::ResourceID<0>::InstanceID_t) * 8;
				...
				template <uint UID>
				explicit Resource (loo::_hidden_::ResourceID<UID> id) :
			...
			struct OnImageDataLoadedEvent
			{
				...
				OnImageDataLoadedEvent (const Callback_t &cb, BytesU size, const loo::math::uint3 &imageSize,
			...
			using TaskName_t = _vkfg_hidden_::TaskName_t;
		...
		public:
			...
			ND_ ShaderDbgIndex  AppendShader (INOUT ArrayView<loo::math::RectI> &, const TaskName_t &name, const _vkfg_hidden_::GraphicsShaderDebugMode &mode, BytesU size = 8_Mb);
			ND_ ShaderDbgIndex  AppendShader (const TaskName_t &name, const _vkfg_hidden_::ComputeShaderDebugMode &mode, BytesU size = 8_Mb);
			ND_ ShaderDbgIndex  AppendShader (const TaskName_t &name, const _vkfg_hidden_::RayTracingShaderDebugMode &mode, BytesU size = 8_Mb);
			...
			ND_ BytesU					GetMaxWritableStoregeSize ()	const { SHAREDLOCK (RWDataRaceCheck,_drCheck);  return _staging.hostWritableBufferSize / 4; }
			ND_ BytesU					GetMaxReadableStorageSize ()	const { SHAREDLOCK (RWDataRaceCheck, _drCheck);  return _staging.hostReadableBufferSize / 4; }


			ND_ EQueueType				GetQueueType ()					const { SHAREDLOCK (RWDataRaceCheck, _drCheck);  return _queueType; }
			ND_ EState					GetState () { return _state.load (memory_order_relaxed); }
			ND_ ArrayView<VCmdBatchPtr>	GetDependencies ()				const { SHAREDLOCK (RWDataRaceCheck, _drCheck);  return _dependencies; }
			ND_ VSubmitted *			GetSubmitted ()					const { SHAREDLOCK (RWDataRaceCheck, _drCheck);  return _submitted; }		// TODO: rename
