...
namespace loo
{
	namespace vkfg
	{
		...
		*/
		...
		=================================================
		*/
		inline void CopyScissors (VCommandBuffer &cb, const _vkfg_hidden_::Scissors_t &inScissors, OUT ArrayView<loo::math::RectI> &outScissors)
		{
			if (inScissors.empty ())
				return;

			auto*	ptr = cb.GetAllocator ().Alloc< loo::math::RectI > (inScissors.size ());
		...
		*/
		...
		=================================================
		*/
		struct VFgTask_VisitFuncLocal
		{
			VkClearColorValue& _clearValue;
			VFgTask_VisitFuncLocal (VkClearColorValue& cv) :_clearValue (cv) {}

			void operator()(const loo::math::RGBA32f &col)const { std::memcpy (_clearValue.float32, &col, sizeof (_clearValue.float32)); }
			void operator()(const loo::math::RGBA32u &col)const { std::memcpy (_clearValue.uint32, &col, sizeof (_clearValue.uint32)); }
			void operator()(const loo::math::RGBA32i &col)const { std::memcpy (_clearValue.int32, &col, sizeof (_clearValue.int32)); }
		...
		{
			//ASSERT( not _dstImage->IsImmutable() );
			VFgTask_VisitFuncLocal visitfunc = VFgTask_VisitFuncLocal (_clearValue);
			Visit (task.clearValue,
				visitfunc
		/*		[&](const loo::math::RGBA32f &col) { std::memcpy (_clearValue.float32, &col, sizeof (_clearValue.float32)); },
				[&](const loo::math::RGBA32u &col) { std::memcpy (_clearValue.uint32, &col, sizeof (_clearValue.uint32)); },
				[&](const loo::math::RGBA32i &col) { std::memcpy (_clearValue.int32, &col, sizeof (_clearValue.int32)); },
				[&](const NullUnion &) {}*/
			);
		}
		...
		*/
			...
			dstBuffer{ cb.ToLocal (task.dstBuffer) }
		{
			...
			_regions = ArrayView<Region>{ dst, cnt };
		}
