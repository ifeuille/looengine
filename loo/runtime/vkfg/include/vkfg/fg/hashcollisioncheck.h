...
namespace loo
{
	namespace vkfg
	{
		...
		public:
			...
			void  Add (const _hidden_::IDWithString<Size, UID, Optimize, Seed> &id);

			template <size_t Size, uint UID, bool Optimize, uint Seed>
			ND_ uint  RecalculateSeed (const _hidden_::IDWithString<Size, UID, Optimize, Seed> &);
		...
		*/
		...
		template <size_t Size, uint UID, bool Optimize, uint Seed>
		inline void  HashCollisionCheck<GuardType>::Add (const _hidden_::IDWithString<Size, UID, Optimize, Seed> &id)
		{
			if /*constexpr*/ (not Optimize)
			{
				EXLOCK (GuardType,_guard);
		...
		*/
		template <typename GuardType>
		template <size_t Size, uint UID, bool Optimize, uint Seed>
		inline uint  HashCollisionCheck<GuardType>::RecalculateSeed (const _hidden_::IDWithString<Size, UID, Optimize, Seed> &)
		{
			SHAREDLOCK (GuardType,_guard);
