...
#include "vkfg/fg/bindingindex.h"
...
namespace loo
{
	namespace vkfg
	{
		...
		public:
			...
			using UniformData_t = nonstd::variant< NullUnion, Texture, Sampler, SubpassInput, Image, UniformBuffer, StorageBuffer, RayTracingScene >;
			...
			using DescriptorSets_t = FixedArray< DescriptorSet, LOO_MaxDescriptorSets >;
			using PushConstants_t = FixedMap< PushConstantID, PushConstant, LOO_MaxPushConstants >;
			...
			using ShaderDataUnion_t = nonstd::variant< NullUnion, SharedShaderPtr<String>, SharedShaderPtr<Array<uint8_t>>, SharedShaderPtr<Array<uint>>, VkShaderPtr >;
			using ShaderDataMap_t = HashMap< EShaderLangFormat, ShaderDataUnion_t >;
			using SpecConstants_t = FixedMap< SpecializationID, uint, LOO_MaxSpecConstants >;
			...
			struct Shader
			{
				...
				Shader () {}
				void AddShaderData (EShaderLangFormat fmt, nonstd::string_view entry, String&& src, StringView dbgName = Default);
				void AddShaderData (EShaderLangFormat fmt, nonstd::string_view entry, Array<uint8_t> &&bin, StringView dbgName = Default);
				void AddShaderData (EShaderLangFormat fmt, nonstd::string_view entry, Array<uint> &&bin, StringView dbgName = Default);
			};
		...
		struct GraphicsPipelineDesc final : PipelineDescription
		{
			...
			struct FragmentOutput
			{
				...
				// methods
				FragmentOutput () {}
				FragmentOutput(const RenderTargetID& id, uint index, EFragOutput type)
			...
			using VertexAttribs_t = FixedArray< VertexAttrib, LOO_MaxVertexAttribs >;
			using FragmentOutputs_t = FixedArray< FragmentOutput, LOO_MaxColorBuffers >;
		...
		struct MeshPipelineDesc final : PipelineDescription
		{
			...
			uint				_maxVertices = 0;
			uint				_maxIndices = 0;
			loo::math::uint3	_defaultTaskGroupSize;
			loo::math::uint3	_taskSizeSpec;
			loo::math::uint3	_defaultMeshGroupSize;
			loo::math::uint3	_meshSizeSpec;
			bool				_earlyFragmentTests = true;
