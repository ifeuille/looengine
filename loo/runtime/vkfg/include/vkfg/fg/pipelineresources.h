...
//
		...
		public:
			...
			void  AllowEmptyResources (bool value) { EXLOCK (RWDataRaceCheck, _drCheck);  _allowEmptyResources = value; }
			...
			ND_ RawDescriptorSetLayoutID	GetLayout ()				const {
				SHAREDLOCK (RWDataRaceCheck, _drCheck); ASSERT (_dataPtr);  return _dataPtr->layoutId; }
			ND_ ArrayView< uint >			GetDynamicOffsets ()		const;
			ND_ bool						IsEmptyResourcesAllowed ()	const { SHAREDLOCK (RWDataRaceCheck, _drCheck);  return _allowEmptyResources; }
			ND_ bool						IsInitialized ()			const { SHAREDLOCK (RWDataRaceCheck, _drCheck);  return _dataPtr != null; }
		...
		using PipelineResourceSet = FixedMap< DescriptorSetID, Ptr<const PipelineResources>, LOO_MaxDescriptorSets >;
		...
		*/
		...
		{
			//std::is_same< std::remove_cv_t<std::remove_reference_t<T>>, DynamicData >::value;
			if (!std::is_same< std::remove_cv_t<std::remove_reference_t<T>>, DynamicData >::value)
			{
				ASSERT (false);
			}
