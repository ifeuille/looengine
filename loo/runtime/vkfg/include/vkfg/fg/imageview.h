...
namespace loo
{
	namespace vkfg
	{
		...
		public:
			...
			using LoadRGBA32fFun_t = void (*) (ArrayView<T>, OUT loo::math::RGBA32f &);
			using LoadRGBA32uFun_t = void (*) (ArrayView<T>, OUT loo::math::RGBA32u &);
			using LoadRGBA32iFun_t = void (*) (ArrayView<T>, OUT loo::math::RGBA32i &);


			// variables
		private:
			BufferView			_parts;
			loo::math::uint3	_dimension;
			size_t				_rowPitch = 0;
		...
		public:
			...
			ImageView (ArrayView<ArrayView<T>> parts, const loo::math::uint3 &dim, BytesU rowPitch, BytesU slicePitch, EPixelFormat format, EImageAspect aspect);
			...
			ND_ loo::math::uint3 const&	Dimension ()	const { return _dimension; }
			...
			ND_ ArrayView<T>  GetPixel (const loo::math::uint3 &point) const
			{
				ASSERT (loo::math::All (point < _dimension));

				return GetRow (point.y, point.z).section ((point.x * _bitsPerPixel) / 8, (_bitsPerPixel + 7) / 8);
			}


			void Load (const loo::math::uint3 &point, OUT loo::math::RGBA32f &col) const
			{
				ASSERT (_loadF4);
				return _loadF4 (GetPixel (point), OUT col);
			}

			void Load (const loo::math::uint3 &point, OUT loo::math::RGBA32u &col) const
			{
				ASSERT (_loadU4);
				return _loadU4 (GetPixel (point), OUT col);
			}

			void Load (const loo::math::uint3 &point, OUT loo::math::RGBA32i &col) const
			{
				ASSERT (_loadI4);
