	...
	namespace vkfg
	{
		...
		*/
		ND_ inline EResourceState  EResourceState_FromShaders (EShaderStages values)
		{
			...
			for (EShaderStages t = EShaderStages (1 << 0); t < EShaderStages::_end; t = EShaderStages (uint (t) << 1))
			{
		...
		*/
		struct PixelFormatInfo
		{
			...
			loo::math::uint2	blockSize = loo::math::uint2 (1, 1);

			constexpr PixelFormatInfo ()/*:blockSize(1,1)*/{}
			...
			constexpr PixelFormatInfo (EPixelFormat fmt, uint bpp, const loo::math::uint2 &size, EType type, EImageAspect aspect = EImageAspect::Color) :
		...
		LOO_BIT_OPERATORS (PixelFormatInfo::EType);
