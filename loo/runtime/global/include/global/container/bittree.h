...
namespace loo
{
	namespace _hidden_
	{
		...
		struct BitTreeImpl< IndexType, BitType, MaxSize, 0 >
		{
			...
			{
				int  idx1 = LooBitScanForward (_highLevel);
				if (idx1 >= 0)
				{
					int  idx0 = LooBitScanForward (_bits[idx1]);
					ASSERT (idx0 >= 0);
		...
		struct BitTreeImpl
		{
			...
			static constexpr IndexType  _CalcLevelSize ()
			{
				...
				if /*constexpr*/ (cb) {
					...
					return loo::math::Min (loo::math::AlignToLarger (MaxSize, IndexType (BitCount)), BitCount*BitCount);
			}

			static constexpr IndexType	MaxLevelSize = _CalcLevelSize< Level - 1 > ();
			static constexpr uint		Size = loo::math::Max (1u, uint ((MaxSize + MaxLevelSize - 1) / MaxLevelSize));
			...
			ND_ bool Assign (OUT Index_t &index)
			{
				int  idx = LooBitScanForward (_highLevel);
				if (idx >= 0)
				{
	...
	public:
		...
		using BitTreeHighLevel_t = _hidden_::BitTreeImpl< IndexType, BitType, MaxSize, LevelCount >;
