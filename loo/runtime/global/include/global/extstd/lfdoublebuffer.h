...
#include "global/math/compilemath.h"
...
namespace loo
{
	...
	public:
		...
		static constexpr uint		Offset = math::CT_IntLog2< MaxItems > +uint (not IsPowerOfTwo (MaxItems));
		...
		// variables
	private:
		alignas(LOO_CACHE_LINE) Atomic_t		_mode;
	...
	public:
		...
		ND_ Value_t  Swap (const uint id, std::memory_order memOrder = std::memory_order_seq_cst)
		{
			...
			} while (not _mode.compare_exchange_weak (expected, next, memOrder, memory_order_relaxed));
