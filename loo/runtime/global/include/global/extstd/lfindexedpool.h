			...
			for (size_t i = 0; i < MaxChunks; ++i)
			{
				...
				if (ptr == null)
				{
					...
					for (ValueChunk_t* expected = null;
						not _values[i].compare_exchange_weak (expected, ptr, memory_order_acquire);)
					{
				...
				for (int index = LooBitScanForward (bits); index >= 0;)
				{
					...
					if (_assignedBits[i].compare_exchange_weak (bits, bits ^ mask, memory_order_acquire, memory_order_relaxed))
					{
