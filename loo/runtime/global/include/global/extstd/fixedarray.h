...
namespace loo
{
	...
	public:
		...
		ND_ constexpr std::size_t			size ()					const { return _count; }
		...
		ND_ constexpr T &				operator [] (std::size_t i) { ASSERT (i < _count);  return _array[i]; }
		ND_ constexpr T const &			operator [] (std::size_t i)	const { ASSERT (i < _count);  return _array[i]; }
		...
		ND_ static constexpr std::size_t		capacity () { return ArraySize; }
		...
		constexpr void  insert (std::size_t pos, T &&value)
		{
			ASSERT (_count < capacity ());
			...
			for (std::size_t i = _count++; i > pos; --i) {
		...
		constexpr void  resize (std::size_t newSize)
		{
			...
			{
				// delete elements
				for (std::size_t i = newSize; i < _count; ++i)
				{
					_array[i].~T ();
				}
		...
		constexpr void  fast_erase (std::size_t index)
		{
			ASSERT (index < _count);
...
{
	template <typename T, std::size_t ArraySize>
	struct hash< loo::FixedArray<T, ArraySize> >
	{
		ND_ std::size_t  operator () (const loo::FixedArray<T, ArraySize> &value) const
		{
			return std::size_t (loo::HashOf (loo::ArrayView<T>{ value }));
		}
	};
