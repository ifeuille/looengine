...
namespace loo
{
	...
	struct alignas(LOO_CACHE_LINE) LfFixedStack
	{
		// variables
	private:
	...
	public:
		...
		{
			const uint	idx = _count.fetch_add (1, std::memory_order_relaxed);
			if (idx < Size) {
				_data[idx] = value;
				return true;
			}
	...
	struct alignas(LOO_CACHE_LINE) LfFixedStack< std::tuple<Types...>, Size >
	{
		// types
	private:
