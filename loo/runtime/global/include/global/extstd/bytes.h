...
namespace loo
{
	...
	*/
	...
	ND_ constexpr inline BytesU  OffsetOf (A B::*member)
	{
		const union U {
			B		b;
			int		tmp;
...
{
	template <typename T>
	struct hash< loo::Bytes<T> >
	{
		ND_ size_t  operator () (const loo::Bytes<T> &value) const
		{
			return size_t (loo::HashOf (T (value)));
		}
	};
