...
namespace loo
{
	...
	public:
		...
		NtBasicStringView (nonstd::basic_string_view<T> str);
		...
		Self& operator = (nonstd::basic_string_view<T>) = delete;
		Self& operator = (const std::basic_string<T> &) = delete;
		Self& operator = (const T*) = delete;

		explicit operator nonstd::string_view ()	const { return nonstd::string_view{ _data, _length }; }
	...
	template <typename T>
	inline NtBasicStringView<T>::NtBasicStringView (nonstd::basic_string_view<T> str) :
