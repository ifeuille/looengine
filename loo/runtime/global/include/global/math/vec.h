...
namespace loo
{
	namespace math
	{
		...
		struct Vec< T, 4 >
		{
			...
			{
				// check is supported cast Vec to array
				/*STATIC_ASSERT*/ASSERT (offsetof (Self, x) + sizeof (T) == offsetof (Self, y));
				/*STATIC_ASSERT*/ASSERT (offsetof (Self, y) + sizeof (T) == offsetof (Self, z));
				/*STATIC_ASSERT*/ASSERT (offsetof (Self, z) + sizeof (T) == offsetof (Self, w));
				/*STATIC_ASSERT*/ ASSERT (sizeof (T)*(size () - 1) == (offsetof (Self, w) - offsetof (Self, x)));
			}
		...
		*/
		template <typename T, uint I>
		ND_ LOO_FORCEINLINE EnableIf<IsFloatPoint<T>, Vec<T, I>>  Ln (const Vec<T, I>& v)
		{
			Vec<T, I>	res;
			for (uint i = 0; i < I; ++i) {
				res[i] = Ln (v[i]);
		...
		template <typename T, uint I>
		ND_ LOO_FORCEINLINE EnableIf<IsFloatPoint<T>, Vec<T, I>>  Log2 (const Vec<T, I>& v)
		{
			Vec<T, I>	res;
			for (uint i = 0; i < I; ++i) {
				res[i] = Log2 (v[i]);
		...
		template <typename T, uint I>
		ND_ LOO_FORCEINLINE EnableIf<IsFloatPoint<T>, Vec<T, I>>  Log10 (const Vec<T, I>& v)
		{
			Vec<T, I>	res;
			for (uint i = 0; i < I; ++i) {
				res[i] = Log10 (v[i]);
		...
		template <typename T, uint I>
		ND_ LOO_FORCEINLINE EnableIf<IsFloatPoint<T>, Vec<T, I>>  Log (const Vec<T, I>& v)
		{
			Vec<T, I>	res;
			for (uint i = 0; i < I; ++i) {
				res[i] = Log (v[i]);
		...
		*/
		template <typename T, uint I>
		LOO_FORCEINLINE EnableIf<IsFloatPoint<T>, Vec<T, I>>  Wrap (const Vec<T, I>& v, const T& minValue, const T& maxValue)
		{
			Vec<T, I>	res;
			for (uint i = 0; i < I; ++i) {
		...
		*/
		template <typename T, uint I>
		ND_ LOO_FORCEINLINE constexpr EnableIf<IsScalar<T> and IsFloatPoint<T>, T>  Round (const Vec<T, I>& v)
		{
			Vec<T, I>	res;
			for (uint i = 0; i < I; ++i) {
				res[i] = Round (v[i]);
		...
		template <typename T, uint I>
		ND_ LOO_FORCEINLINE constexpr auto  RoundToInt (const Vec<T, I>& v)
		{
		...
		template <typename T, uint I>
		ND_ LOO_FORCEINLINE constexpr auto  RoundToUint (const Vec<T, I>& v)
		{
...
#if LOO_FAST_HASH
	template <typename T, uint I>
	struct hash< loo::math::Vec<T, I> > {
		ND_ size_t  operator () (const loo::math::Vec<T, I> &value) const {
...
#else
	template <typename T>
	struct hash< loo::math::Vec<T, 2> > {
		ND_ size_t  operator () (const loo::math::Vec<T, 2> &value) const {
			return size_t (loo::HashOf (value.x) + loo::HashOf (value.y));
		}
	};

	template <typename T>
	struct hash< loo::math::Vec<T, 3> > {
		ND_ size_t  operator () (const loo::math::Vec<T, 3> &value) const {
			return size_t (loo::HashOf (value.x) + loo::HashOf (value.y) + loo::HashOf (value.z));
		}
	};

	template <typename T>
	struct hash< loo::math::Vec<T, 4> > {
		ND_ size_t  operator () (const loo::math::Vec<T, 4> &value) const {
