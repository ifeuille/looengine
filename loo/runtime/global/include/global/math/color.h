	...
	namespace math
	{
		...
		struct RGBAColor
		{
			...
			constexpr RGBAColor () : r{ T (0) }, g{ T (0) }, b{ T (0) }, a{ T (0) }
			{
				...
				/*STATIC_ASSERT*/ ASSERT (offsetof (Self, r) + sizeof (T) == offsetof (Self, g));
				/*STATIC_ASSERT*/ ASSERT (offsetof (Self, g) + sizeof (T) == offsetof (Self, b));
				/*STATIC_ASSERT*/ ASSERT (offsetof (Self, b) + sizeof (T) == offsetof (Self, a));
				/*STATIC_ASSERT*/ ASSERT (sizeof (T)*(size () - 1) == (offsetof (Self, a) - offsetof (Self, r)));
			}
		...
		*/
		template <typename T>
		inline constexpr bool4  Equals (const RGBAColor<T> &lhs, const RGBAColor<T> &rhs, const T &err = std::numeric_limits<T>::epsilon () * T (2))
		{
