...
#define PANIC_PRECONDITION(format, ...)                                                            \
    ::utils::PreconditionPanic::panic(__FUNC__,                                         \
...
#define PANIC_POSTCONDITION(format, ...)                                                           \
    ::utils::PostconditionPanic::panic(__FUNC__,                                        \
            PANIC_FILE(__FILE__), __LINE__, format, ##__VA_ARGS__)
...
#define PANIC_ARITHMETIC(format, ...)                                                              \
    ::utils::ArithmeticPanic::panic(__FUNC__,                                           \
            PANIC_FILE(__FILE__), __LINE__, format, ##__VA_ARGS__)
...
#define PANIC_LOG(format, ...)                                                                     \
    ::utils::details::logAndPanic(__FUNC__,                                             \
            PANIC_FILE(__FILE__), __LINE__, format, ##__VA_ARGS__)
...
#define ASSERT_PRECONDITION(cond, format, ...)                                                     \
    (!UNLIKELY(cond) ? PANIC_PRECONDITION(format, ##__VA_ARGS__) : (void)0)

#if defined(UTILS_EXCEPTIONS) || !defined(NDEBUG)
#define ASSERT_PRECONDITION_NON_FATAL(cond, format, ...)                                           \
    (!UNLIKELY(cond) ? PANIC_PRECONDITION(format, ##__VA_ARGS__), false : true)
#else
#define ASSERT_PRECONDITION_NON_FATAL(cond, format, ...)                                           \
    (!UNLIKELY(cond) ? PANIC_LOG(format, ##__VA_ARGS__), false : true)
#endif
...
#define ASSERT_POSTCONDITION(cond, format, ...)                                                    \
    (!UNLIKELY(cond) ? PANIC_POSTCONDITION(format, ##__VA_ARGS__) : (void)0)

#if defined(UTILS_EXCEPTIONS) || !defined(NDEBUG)
#define ASSERT_POSTCONDITION_NON_FATAL(cond, format, ...)                                          \
    (!UNLIKELY(cond) ? PANIC_POSTCONDITION(format, ##__VA_ARGS__), false : true)
#else
#define ASSERT_POSTCONDITION_NON_FATAL(cond, format, ...)                                          \
    (!UNLIKELY(cond) ? PANIC_LOG(format, ##__VA_ARGS__), false : true)
#endif
...
#define ASSERT_ARITHMETIC_NON_FATAL(cond, format, ...)                                             \
    (!UNLIKELY(cond) ? PANIC_ARITHMETIC(format, ##__VA_ARGS__), false : true)
#else
#define ASSERT_ARITHMETIC_NON_FATAL(cond, format, ...)                                             \
    (!UNLIKELY(cond) ? PANIC_LOG(format, ##__VA_ARGS__), false : true)
#endif
