...
// backslash-newlines into newlines
		...
		if (!macro.isFunction) {
			...
			if (!symbols.test (PP_LPAREN)) {
				*macroName = QByteArray ();
				...
				syms.push_back (s);
				syms.rbegin ()->lineNum = lineNum;
				return syms;
			}
			std::vector<Symbols> arguments (5);
			...
			for (int i = 0; i < macro.symbols.size (); ++i) {
				...
				int index = -1;// macro.arguments.indexOf (s);
				auto it = std::find (macro.arguments.begin (), macro.arguments.end (), s);
				if (it == macro.arguments.end ())
				{
					index = -1;
				}
				else
				{
					index = it - macro.arguments.begin ();
				}
				if (mode == Normal) {
					if (index >= 0 && index < arguments.size ()) {
						...
						if (i == macro.symbols.size () - 1 || macro.symbols.at (i + 1).token != PP_HASHHASH) {
							...
							loo::Symbols& symbols = arguments.at (index);
							expansion.insert (expansion.end (), symbols.begin (), symbols.end ());
							//expansion.push_back(arguments.at (index));
						}
					}
					else {
					...
					StringReplace (stringified, '"', "\\\"");//stringified.replace ('"', "\\\"");
					stringified.insert (stringified.begin (), '"');
					stringified.push_back ('"');//stringified.append('"');
					...
					while (expansion.size () && expansion.rbegin ()->token == PP_WHITESPACE)
						expansion.pop_back ();
					...
					if (!expansion.empty () && expansion.rbegin ()->token == s.token
						&& expansion.rbegin ()->token != STRING_LITERAL) {
						Symbol last = *expansion.rbegin ();
	...
	void Preprocessor::macroExpand (Symbols * into, Preprocessor * that, const Symbols & toExpand, int & index, int lineNum, bool one, const std::set<QByteArray>& excludeSymbols)
	{
		SymbolStack symbols;
		SafeSymbols sf;
		sf.symbols = toExpand;
	...
	Symbols Preprocessor::tokenize (const QByteArray & input, int lineNum, TokenizeMode mode)
	{
		Symbols symbols;
			...
			if (mode == TokenizeCpp || mode == TokenizeDefine) {
				...
				if (token > SPECIAL_TREATMENT_MARK) {
					...
					case QUOTE:
						...
						if (!Preprocessor::preprocessOnly
							&& !symbols.empty ()
							&& (*symbols.rbegin ()).token == STRING_LITERAL) {

							const QByteArray newString
								= '\"'
								+ (*symbols.rbegin ()).unquotedLexem ()
								+ input.substr (lexem - begin + 1, data - lexem - 2)
								+ '\"';
							*symbols.rbegin () = Symbol ((*symbols.rbegin ()).lineNum,
								STRING_LITERAL,
								newString);
							continue;
						}
	...
	int PP_Expression::primary_expression ()
	{
		...
		if (test (PP_LPAREN)) {
			...
			value = toIntegral_helper<int> (lexem ().c_str(), 0, 0);
		}
		return value;
	}
	...
	*/
	...
	static QByteArray searchIncludePaths (const std::vector<Parser::IncludePath> &includepaths,
		const QByteArray &include)
	{
		...
		for (int j = 0; j < includepaths.size () && !fi.exists (); ++j) {
			...
			/*	fi.setFile (QString::fromLocal8Bit (p.path + '/' + include.left (slashPos) + ".framework/Headers/"),
				...
				fi.assign (fs::combine_path_file (p.path, include));
		...
		if (!fi.exists () || fi.is_directory ())
			return QByteArray ();
		fi.path ().filename ().string();
	...
	QByteArray Preprocessor::resolveInclude (const QByteArray & include, const QByteArray & relativeTo)
	{
		if (!relativeTo.empty ()) {
			//QFileInfo fi;
			fs::directory_entry fi;
			fi.assign (fs::combine_path_file (relativeTo, include));
	...
	void Preprocessor::preprocess (const QByteArray & filename, Symbols & preprocessed)
	{
		...
		while (hasNext ()) {
			...
			case PP_INCLUDE:
			{
				...
				if (include.empty ())//?include.isNull () 这可能是错得
					continue;
				...
				fs::ifstream file (include.data (), std::ios::in | std::ios::binary | std::ios::ate);//?
				if (!file.is_open ())
				{
					continue;
				}

				QByteArray input = readFile2String (file);
			...
			case PP_DEFINE:
			{
				...
				while (!macro.symbols.empty () &&
					(macro.symbols.rbegin ()->token == PP_WHITESPACE || macro.symbols.rbegin ()->token == WHITESPACE))
					macro.symbols.pop_back ();

				if (!macro.symbols.empty ()) {
					if (macro.symbols.begin ()->token == PP_HASHHASH ||
						macro.symbols.rbegin ()->token == PP_HASHHASH) {
						error ("'##' cannot appear at either end of a macro expansion");
					}
				}
				macros.insert (std::make_pair(name, macro));
				continue;
			}
			case PP_UNDEF: {
				next ();
	...
	Symbols Preprocessor::preprocessed (const QByteArray & filename, fs::ifstream & device)
	{
		...
		result.reserve (device.tellg () / 300000);
